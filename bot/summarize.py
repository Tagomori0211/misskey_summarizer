...
特に引用すべきと判断した場合は「>[ユーザー名]さん 引用内容」として引用を行うこと。
---
[テキスト]
"""
...
## 指示
#必ず順守しなければならない条件
要約した最終的な回答は2900文字以内に収めることこれは絶対とする。
...
## ★★★ 最重要ルール (文字数制限) ★★★
Misskeyの投稿制限（約3000文字）を考慮し、
レポートの**最終的な全文（見出しやMFMタグ含む）が、必ず2500文字以内**に収まるように、**非常に簡潔に**要約してください。
文字数を守るためなら、一部のマイナーなトピックを省略しても構いません。

## MFMの例
- 見出し: `$[tada.speed=0s 見出し]`(0sの後ろには必ず半角スペースを入れること)
- 太字: `**太字**`
- 引用: `>[ユーザー名]さん　引用内容`
なお可読性の観点からMFMには実装されているがx2,x3,x4の使用は禁止する。
## 要約の断片群
---
[テキスト]
---
